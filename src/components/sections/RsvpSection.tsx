...
export function RsvpSection() {
  ...
  const [nameFromUrl, setNameFromUrl] = useState(false);
  ...
  useEffect(() => {
    ...
    const guestName = decodeURIComponent(pathName)
      .replace(/[-_]+/g, " ")
      .trim()
      .slice(0, 80);
  ...
  useEffect(() => {
    fetchHeader()
      ...
      .catch((error) => console.error("Gagal memuat tanggal RSVP dari Firebase:", error));
  }, []);
        ...
        <Reveal direction="up">
          <div className="mb-12 flex flex-col items-center gap-3 text-center">
            ...
            <p className="font-sans text-sm text-[color:var(--text-secondary)] max-w-sm text-balance">
              Mohon konfirmasi kehadiran Anda untuk acara{" "}
              <strong className="text-[#D4AF37]">{header.dateText}</strong>.
            </p>
          </div>
        </Reveal>
            ...
            <AnimatePresence mode="wait">
              {state.status === "success" ? (
                ...
                <div className="py-8 text-center">
                  <h3 className="font-serif text-xl font-semibold text-[color:var(--text-primary)]">RSVP tidak tersedia</h3>
                  <p className="mt-2 text-sm text-[color:var(--text-secondary)]">Gunakan link undangan pribadi yang terdaftar untuk mengirim konfirmasi.</p>
                </div>
              ) : (
                <motion.form
                  key="form"
